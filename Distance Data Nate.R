...
binned_data <- def_distances_tbl %>% mutate(def_distance_bin = 
                                                    case_when(
                                                      mean_defender_distance < 25 ~ 1,
                                                      mean_defender_distance < 30 ~ 2,
                                                      mean_defender_distance < 35 ~ 3,
...
result_table_epa
ggplot(data = result_table_epa %>% filter(event.y != "punt_muffed"), mapping = aes(x = yardlinebin, y = def_distance_bin, 
                                                                                   fill = event.y, alpha = median_epa)) + 
...
# Get mean epa for each group
wpa_tbl <- binned_data %>% group_by(yardlinebin, def_distance_bin, event.y) %>% 
  mutate(median_wpa = mean(-wpa)) %>% 
...
table(paste0(wpa_tbl$def_distance_bin,"-", wpa_tbl$yardlinebin))
